...
public class MethodStudy {
    ...
    */
     ...
     */
    public static void main(String[] args) {    //여기서부터 구현부
        sum(5,7); //sum 메소드 호출  //정확히는 class명을 붙여야하므로 MethodStudy.sum()이 되어야한다. 생략이 가능할뿐
        sum(15,17); //sum 메소드 호출  //파라미터 선언 자체가 값이 될 순 없다. 그러나 main을 통해 호출된다.
        int result=returnsum(5,7);   //void 값을 return 처리하는 함수를 넣어준다.
        System.out.println("result: "+result);
        System.out.println("result: "+returnsum(15,17));  //아래의 출력함수와 값이 같다.
        System.out.println("result2: "+32);
    }

    //sum이라는 이름의 static 메소드를 정의했다.(파라미터 2개)
    static void sum(int n1, int n2) { //파라미터(변수)마다 타입을 전부 적어야하고 갯수도 맞춰야한다. 이 파라미터들은 호출되는 순간 사라진다.
